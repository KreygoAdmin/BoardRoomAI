  ...
  <div className="flex-1 flex flex-col bg-gray-950 relative w-full min-h-0">
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-thin scrollbar-thumb-gray-800 relative">
      ...
      {isProcessing && !retryStatus && (() => {
        const speakingMember = processingStage
          ? boardMembers.find(m => processingStage.toLowerCase().includes(m.name.toLowerCase()))
          : null;
        return (
          <div className="flex items-center gap-3 mt-4 ml-2 px-1">
            {speakingMember ? (
              <>
                <div className={`w-8 h-8 rounded-full ${speakingMember.avatar} flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-lg`}>
                  {speakingMember.name[0]}
                </div>
                <div className="text-sm text-gray-400 flex items-center gap-1.5">
                  <span className="font-semibold text-gray-200">{speakingMember.name}</span>
                  <span className="text-gray-500">({speakingMember.role})</span>
